...
__device__ int drawFigure(PlanePart* plane, Circle* circle, size_t* rand_seed) {
    if (CIRCLE_COLOR == circle->type) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            drawLine(plane, (size_t) (circle->x + circle->r * x), 
                            (size_t) (circle->y + circle->r * y),
                            (size_t) (circle->x + circle->r * x1), 
                            (size_t) (circle->y + circle->r * y1));
            x = x1;
            y = y1;
        }
    }
...
__global__ void create_data_gpu(PlanePart* plane, 
...
err_t create_data(PlanePart* plane, FigureCount desired, FigureCount* actual, size_t seed) {
